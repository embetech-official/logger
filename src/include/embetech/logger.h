...
#define LOGGER_LEVEL_ALERT                                                                                                                           \
  1 ///< Syslog compliant level with description: A condition that should be corrected immediately, such as a corrupted system database
...
#define LOGGER_LEVEL_NOTICE                                                                                                                          \
  5 ///< Syslog compliant level with description: Conditions that are not error conditions, but that may require special handling
#define LOGGER_LEVEL_INFO 6 ///< Syslog compliant info level
#define LOGGER_LEVEL_VERBOSE                                                                                                                         \
  7 ///< Additional verbosity level for informational messages that displaying is optional and yield significant performance penalty
#define LOGGER_LEVEL_DEBUG                                                                                                                           \
  8 ///< Syslog compliant level with description: Messages that contain information normally of use only when debugging a program
...
#define LOGGER_DISABLED(...)                                                                                                                         \
  LOGGER_PRINT_LINE(LOGGER_LEVEL_DISABLED, __VA_ARGS__) ///< Disables printing message. Useful during development, to leave some test information in
                                                        ///< code without memory/performance penalties
 ...
 * If user needs to print composite log message (i.e. has overloaded print function that uses LOGGER), this macro might be used to construct inline
 * message:
 * @param[in] level predefined log level macro
 * @note Usage example: LOGGER_START(level); {command sequence that uses LOGGER_CONTINUE macro}; LOGGER_END(); //The example above will print single
 * LOGGER header.
 * @note Declaring variables inside LOGGER_START/LOGGER_END scope reduces their scope and lifetime. However, it might be beneficial if the variables
 * are user only for logging (they will be optimized out if log is disabled)
...
/**
 * @brief Binds function to be invoked after each LOGGER message (LOGGER_INFO, LOGGER_NOTICE, etc.), or after each LOGGER Scope
 * (LOGGER_START/LOGGER_END)
 ...
 * @note if LOGGER_RUNTIME_VERBOSITY is set to 0, the function can be called, however the result of the function is not meaningful (runtime level
 ...
 * @param[in] data memory location with at least length size that stores the prefix. LOGGER stores only the pointer and size information, so user MUST
 * guarantee the lifetime of the object
 ...
 *
 * The data MAY NOT be a valid. null-terminated string, and LOGGER guarantees that exactly length characters will be printed out
 * @param[in] data memory location with at least length size that stores the suffix. LOGGER stores only the pointer and size information, so user MUST
 * guarantee the lifetime of the object
...
/****************************************************************************************************************************************************************************/
/************* The content below must NOT be altered (unless you are a developer or the almighty guinea pig). Functions below are not meant to be used
 * by user *************/
/****************************************************************************************************************************************************************************/
...
#ifndef LOGGER_FILE
#define LOGGER_FILE                                                                                                                                  \
  __FILE__ ///< Provides string with source file basename (if possible and supported by build system). If not, falls back to compiler's default
           ///< __FILE__ definition
#endif
...
#define LOGGER_CHANNEL_LEVEL                                                                                                                         \
  LOGGER_CAT(LOGGER_CHANNEL, _LOG_CHANNEL_LEVEL) ///< Normally. this macro shall store integer from ***_LOG_CHANNEL_LEVEL macro.
                                                 ///< If user messed up and defined ***_LOG_CHANNEL_LEVEL empty, it would be empty (we have check for
                                                 ///< this below) If user messed up and did not defined ***_LOG_CHANNEL_LEVEL, it would have
                                                 ///< ***_LOG_CHANNEL_LEVEL value (we also have check for this below)
...
#if LOGGER_NOT(LOGGER_IF_NOT_EMPTY(LOGGER_CHANNEL_LEVEL)) // First ensure that LOGGER_CHANNEL_LEVEL has value. It won't be true if the user messed up
                                                          // and defined ***_LOG_CHANNEL_LEVEL empty
...
#if !LOGGER_CHANNEL_LEVEL &&                                                                                                                         \
    LOGGER_IF_NZ(LOGGER_CHANNEL_LEVEL) // If ***_LOG_CHANNEL_LEVEL was not defined, the LOGGER_CHANNEL_LEVEL will be 0, but direct comparison to 0
                                       // will yield false (because LOGGER_CHANNEL_LEVEL has no explicit value)
 ...
 * @return LOGGER_level
 */
static inline LOGGER_level LOGGER_DoGetRuntimeLevel(void) { return LOGGER_CHANNEL_LEVEL; }
#endif

static inline LOGGER_level LOGGER_GetRuntimeLevel(void) { return LOGGER_DoGetRuntimeLevel(); }
 ...
 * @brief NO-OP
 */
static inline bool LOGGER_Lock(void) { return true; }
